import type { PersistCodec, PersistMigration, PersistOptions } from "./types.js";
...
export type RestorePlan<Value> =
  | { readonly kind: "value"; readonly value: Value }
  | MigrationPlan<Value>;

export type MigrationPlan<Value> = {
  readonly kind: "migration";
  readonly writeBack: string;
  readonly finalize: () => Value;
};

type PlanVersionedRestoreParams<Value> = {
...
export function planVersionedRestore<Value>({
  rawValue,
  initialValue,
  key,
  version,
  migrations,
  codec,
}: PlanVersionedRestoreParams<Value>): RestorePlan<Value> {
  ...
  if (record.version === version) {
    return {
      kind: "value",
      value: decodeVersioned(codec, record.payload, initialValue, key, version),
    };
  }
  ...
  return {
    kind: "migration",
    writeBack: createEnvelope(version, payload),
    finalize: () => decodeVersioned(codec, payload, initialValue, key, version),
  };
}

export function planUnversionedRestore<Value>(
  codec: PersistCodec,
  rawValue: string,
  initialValue: Value,
  key: string,
  ...
  try {
    return { kind: "value", value: codec.decode(rawValue, initialValue) };
  } catch (error) {
...
export function encodeVersioned(
  ...
  return createEnvelope(version, encodePayload(codec, value, key, version));
}

function createEnvelope(version: number, payload: string): string {
