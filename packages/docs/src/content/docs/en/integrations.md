...
Choose a copy-ready path for the task first, then use the compatibility matrix as version-range evidence:
...
| React Native / Expo       | `@zhuangtai-js/react`  | Use the React adapter; the app supplies its own storage             |
...
These release lines and peer ranges match the package manifests. Use them as secondary evidence after choosing a quick start. A 0.x package does not automatically accept an undeclared future Core minor or framework major.
...
- `@zhuangtai-js/persist` reads browser `localStorage` by default and also accepts custom storage methods that return `PromiseLike` values. Server code should pass explicit storage or create the persisted atom only on the client.
